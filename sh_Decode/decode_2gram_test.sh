...
#可变
SCP="/mnt/shareEx/panjingshen/ElderVoice/DATA/scripts/scp_test/version3/Elder"
LM="/mnt/shareEx/panjingshen/ElderVoice/LM/EV2gram_v2/lms/elder2gram_Merge_notSR"
...
for i in `seq 1 $jobs`
do
	srun -p pBatch_all -w aimsl-124 $hdecodeBin -T 1 -A -D -V -C $Config -H $MMF -i ${DIRRESULTTEMP}/sub${i}.mlf -t $PruneBeamWidth1 $PruneBeamWidth2 -u $MaxModelPruning -v $WordendBeanWidth -s $LMScale -p $WordInsertionPenalty -n 24 -w $LM $DCT $HMMList -S ${SCP}_sub${i}.scp > ${DIRLOGTEMP}/sub${i}.log &
#    sleep 5
done
#wait
#rm -f ${SCP}_sub*.scp
...
# /mnt/shareEx/yanquanlei/OLD/work/DNN_audio